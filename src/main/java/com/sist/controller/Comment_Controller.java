...
public class Comment_Controller {
	...
	public ResponseEntity getboard_List(Content_CommentDTO dto, HttpServletRequest request,
		...
		List<MemberDTO> comment_list = this.b_ContentService.getcomment_list(dto);
	
		if (comment_list.size() > 0) {
			for (int i = 0; i < comment_list.size(); i++) {
				for (int j = 0; j < comment_list.get(0).getComments().size(); j++) {
					HashMap hm = new HashMap();
					hm.put("content", comment_list.get(i).getComments().get(j).getCc_Cont());
					hm.put("nickname", comment_list.get(i).getM_Nickname());
					hm.put("cc_no", comment_list.get(i).getComments().get(j).getCc_No());
					hm.put("cc_Indent", comment_list.get(i).getComments().get(j).getCc_Indent());
					hm.put("cc_step", comment_list.get(i).getComments().get(j).getCc_Step());
					hm.put("cc_group", comment_list.get(i).getComments().get(j).getCc_Group());
					hm.put("cc_Date", comment_list.get(i).getComments().get(j).getCc_Date().substring(0, 11));
					hm.put("bc_no", comment_list.get(i).getComments().get(j).getBc_No());
					hm.put("picture", comment_list.get(i).getM_Picture());
